...
function attentionBadge(attention) {
    if (attention === "underperforming") {
        return (_jsx(Badge, { variant: "warning", className: "mt-1 w-fit justify-center whitespace-nowrap text-center", children: "Needs Attention" }));
    }
    return null;
}
...
function DraftActionFooter({ stagedCount, onDiscard, onPublish, scopeLabel, extraActionLabel, onExtraAction, extraActionDisabled, }) {
    ...
    return (_jsx("footer", { className: "fixed inset-x-0 bottom-0 z-[70] border-t bg-card/95 backdrop-blur", children: _jsxs("div", { className: "mx-auto flex max-w-[1450px] flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8", children: [_jsx("p", { className: "text-sm text-muted-foreground", children: hasChanges
                        ? `${stagedCount} staged change${stagedCount === 1 ? "" : "s"}${scopeSuffix} pending publish.`
                        : `No staged changes${scopeSuffix}.` }), _jsxs("div", { className: "flex items-center gap-2", children: [extraActionLabel && onExtraAction && (_jsx(Button, { variant: "secondary", onClick: onExtraAction, disabled: extraActionDisabled, children: extraActionLabel })), _jsx(Button, { variant: "outline", onClick: onDiscard, disabled: !hasChanges, children: "Discard Draft" }), _jsx(Button, { onClick: onPublish, disabled: !hasChanges, children: `Publish Changes (${stagedCount})` })] })] }) }));
}
...
export default function App() {
    ...
    const sortIconForKey = (key) => {
        if (sortState.key !== key) {
            return _jsx(ArrowUpDown, { className: "h-3.5 w-3.5 text-muted-foreground" });
        }
        return sortState.direction === "asc" ? (_jsx(ArrowUp, { className: "h-3.5 w-3.5 text-primary" })) : (_jsx(ArrowDown, { className: "h-3.5 w-3.5 text-primary" }));
    };
